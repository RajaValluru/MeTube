...
$servername = "mysql1.cs.clemson.edu";
  ...
  if(isset($_REQUEST['delete']))
  {
    $sql = "DELETE FROM messages
            WHERE messageid = ". $_REQUEST['messageid'].";";
...
$servername = "mysql1.cs.clemson.edu";
  ...
  $sql = "SELECT fname,lname, email, subject, message, time, messageid
          FROM (users join messages ON Userid = from_user), blocklist
          WHERE to_user=$userid AND NOT (b_userid=from_user AND O_userid=$userid)
          ORDER BY time DESC;";
  $result = $conn->query($sql);
