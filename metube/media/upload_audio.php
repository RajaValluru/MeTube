...
<html>
	...
	<?php
		...
		echo '<div class="container">';
	        echo '<form action="upload_audio.php" method="post" enctype="multipart/form-data">';
		echo '    <h3>Name</h3>';
		...
		echo '    <select id="channel" name="channel">';
			...
			while($row = mysqli_fetch_assoc($result)) {
			    echo '    <option value="' . $row['AChannel_ID'] . '">' . $row['ChannelName'] . '</option>';
			}
		    }
		echo '    </select>';
		}
		...
		echo '</div>';
		    ...
		    //Do the SQL
				...
				chmod($target_file, 0644);
		                $sql = "INSERT INTO audiolist (AChannel_ID, Name, Path, Size, Format, AccessType, Views, Description) VALUES ( " . $achannel_id . " , '" .
					 $mediaName . "' , '/" . $target_file . "' , " . $fileSize . " , '" . $fileType . "' , '" . $accesstype . "', 0, '" . $description . "');";
				$result = mysqli_query($db_connection, $sql);
				if (!$result)
				{
				...
				foreach ($tags as $tag)
				{
				    $sql = "insert into audiotags values (" . $key . ", '" . $tag . "');";
				    $result = mysqli_query($db_connection, $sql);
				    if (!$result)
				    {
