...
if(isset($_GET['cid']) ){
  ...
  {
    $sql = "DELETE FROM audiolist
            WHERE Audio_id = ". $_REQUEST['audioid'].";";
...
<?php
  ...
  $sql = "SELECT * from audiolist where AChannel_ID=$channelid ORDER BY timestamp desc"; 
  $result = $conn->query($sql);
  ...
  if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
      ...
      echo "<td><form action='../media/view_media.php' method='post'><input type='hidden' name='media_id' value='".$row['Audio_ID']."'><input type='hidden' name='mediatype' value='audio'><input type='submit' value='View'></form> </td>";
      echo "<td><form method='post'> <input type='submit' name='delete' value='Delete'>
            <input type='hidden' name='audioid' value='".$row['Audio_ID']."'>
       </form></td>";
      echo "</tr>";
