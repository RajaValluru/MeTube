...
if($_SESSION['userid'] == '1' or !isset($_SESSION['userid'])){
  ...
  {
    $sql = "DELETE FROM videochannel
            WHERE VChannel_id = ". $_REQUEST['channelid'].";";
...
<?php
  ...
  if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
      ...
      echo "<td><a href='channelpage.php?cid=".$row['VChannel_ID']."'>" . $row['ChannelName'] ."</a></td>";
      echo "<td>".$row['SubscriptionCount']."</td>";
      echo "<td><form method='post'> <input type='submit' name='delete' value='Delete'>
            <input type='hidden' name='channelid' value='".$row['VChannel_ID']."'>
       </form></td>";
      echo "</tr>";
