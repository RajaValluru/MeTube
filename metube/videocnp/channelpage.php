...
if(isset($_GET['cid']) ){
  ...
  $sql = "SELECT * 
          FROM videochannel natural join users
  ...
  while($row = $result->fetch_assoc()) {
    ...
    if($ownerid!=$userid){
      header("Location: otherchannelpage.php?cid=$channelid");
    }
  }
  ...
  {
    $sql = "DELETE FROM videolist
            WHERE Video_id = ". $_REQUEST['videoid'].";";
...
<body>
 <div class="navbar">
  ...
  <a href="createchannel.php">Create Video Channel</a>
  <a href="viewchannels.php">View Your Video Channels</a>
...
<?php
  ...
  $sql = "SELECT * from videolist where VChannel_ID=$channelid ORDER BY timestamp desc"; 
  $result = $conn->query($sql);
  ...
  if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
      ...
      echo "<td><form action='../media/view_media.php' method='post'><input type='hidden' name='media_id' value='".$row['Video_ID']."'><input type='hidden' name='mediatype' value='videos'><input type='submit' value='View'></form> </td>";
